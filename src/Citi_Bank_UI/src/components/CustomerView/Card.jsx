...
const Card = ({
    ...
    const fetchCustomerData = async () => {
      try {
        ...
        const response = await fetch(
          `http://127.0.0.1:5000/recommend?customer_id=CUST0001&view_type=customer`,
          {
            method: "GET",
          }
        );
 ...
 <div className="flex flex-wrap justify-center items-center mb-4 w-full">
  ...
  {/* Additional Content */}
  <div className="bg-blue-900/30 w-full max-w-sm rounded-lg shadow-lg p-4 mt-4" style={{ height: '400px', overflowY: 'auto' }}>
    <h3 className="font-bold mb-2 text-center text-blue-700">✨ Key Benefits</h3>
    <ul className="space-y-2">
      {benefits.map((benefit, index) => (
        <li key={index} className="flex items-center bg-white rounded-md p-2 shadow-sm">
  ...
  {/* Reason Box */}
   <div className="mt-4 w-full max-w-sm h-20">
    {loading ? (
